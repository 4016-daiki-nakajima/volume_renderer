...
/// Anisotropic generalization of GTR2/GGX
inline Real GTR2_aniso(const Vector3 &h_local, Real alpha_x, Real alpha_y) {
    Real hx2 = h_local.x * h_local.x;
    Real hy2 = h_local.y * h_local.y;
    Real hz2 = h_local.z * h_local.z;
    Real t = hx2 / (alpha_x * alpha_x) + hy2 / (alpha_y * alpha_y) + hz2;
    return 1 / (c_PI * alpha_x * alpha_y * t * t);
}

/// Anisotropic Smith masking 
inline Real smith_masking_aniso(const Vector3 &v_local, Real alpha_x, Real alpha_y) {
    Real ax2 = (v_local.x * alpha_x) * (v_local.x * alpha_x);
    Real ay2 = (v_local.y * alpha_y) * (v_local.y * alpha_y);
    Real Lambda = (sqrt(1 + (ax2 + ay2) / (v_local.z * v_local.z)) - 1) / 2;
    return 1 / (1 + Lambda);
}

/// Anisotropic generalization of Heitz's visible normal sampling
inline Vector3 sample_visible_normals_aniso(const Vector3 &local_dir_in,
                                             Real alpha_x, Real alpha_y,
                                             const Vector2 &rnd_param) {
    if (local_dir_in.z < 0) {
        return -sample_visible_normals_aniso(-local_dir_in, alpha_x, alpha_y, rnd_param);
    }
...
/// Burley's roughness/anisotropic reparameterization
inline void disney_aniso_alphas(Real roughness, Real anisotropic, Real &alpha_x, Real &alpha_y) {
    Real aspect = sqrt(1 - Real(0.9) * anisotropic);
    Real alpha_min = Real(0.0001);
    alpha_x = std::max(alpha_min, roughness * roughness / aspect);
    alpha_y = std::max(alpha_min, roughness * roughness * aspect);
}

/// Burley's modified isotropic GTR1 normal distribution.
inline Real GTR1(Real n_dot_h, Real alpha_g) {
    Real alpha2 = alpha_g * alpha_g;
    return (alpha2 - 1) /
        (c_PI * log(alpha2) * (1 + (alpha2 - 1) * n_dot_h * n_dot_h));
}

/// Smith masking with a fixed roughness of 0.25
inline Real smith_masking_clearcoat(const Vector3 &v_local) {
    Real ax2 = (v_local.x * Real(0.25)) * (v_local.x * Real(0.25));
    Real ay2 = (v_local.y * Real(0.25)) * (v_local.y * Real(0.25));
    Real Lambda = (sqrt(1 + (ax2 + ay2) / (v_local.z * v_local.z)) - 1) / 2;
    return 1 / (1 + Lambda);
}

inline Real clearcoat_alpha_g(Real clearcoat_gloss) {
    return (1 - clearcoat_gloss) * Real(0.1) + clearcoat_gloss * Real(0.001);
}
